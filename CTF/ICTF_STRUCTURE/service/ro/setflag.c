...
//int main(int argc, char* argv[])
{
	/*if (argc == 3)			Encryption (not needed?)
	{
		...
		{
			char* tmp = (char*)malloc(sizeof(char)*2000);
			sprintf(tmp, "openssl aes-256-cbc -in -out %s.enc -pass pass:Â§acf578?#*+-463-{{}av@wer637,,..", argv[2], argv[2]);
			popen(tmp, "w");
			return 0;
		}
	...
	encryptedData[0] = (char*)malloc(sizeof(char)*500);
	char line[1024];
	while(fgets(line, 1024, output)!=NULL)
	{
		...
		i++;
		encryptedData[i] = (char*)malloc(sizeof(char)*500);
		free(tmp);
	}
	if(strlen(result) == 0)
